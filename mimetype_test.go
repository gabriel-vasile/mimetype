...
var testcases = []testcase{
	...
	{"16sv", "FORM\x00\x00\x00\x0016SV\x00", "audio/x-16sv", one},
	{"7z", "\x37\x7A\xBC\xAF\x27\x1C", "application/x-7z-compressed", all},
	{"8svx", "FORM\x00\x00\x00\x008SVX\x00", "audio/x-8svx", one},
	...
	{"adf", "DOS\x00", "application/x-amiga-disk-format", one},
	{"ahx", "THX\x00", "audio/x-ahx", one},
	{"aifc", "\x46\x4F\x52\x4D\x00\x00\x00\x00\x41\x49\x46\x43\x00", "audio/x-aifc", one},
	{"aiff", "\x46\x4F\x52\x4D\x00\x00\x00\x00\x41\x49\x46\x46\x00", "audio/aiff", one},
	{"amf", `<?xml version="1.0"?><amf>`, "application/x-amf", one},
	{"amigaExe", "\x00\x00\x03\xF3", "application/x-amiga-executable", one},
	{"amr", "\x23\x21\x41\x4D\x52", "audio/amr", one},
	{"anim", "FORM\x00\x00\x00\x00ANIM\x00", "video/x-anim", one},
	...
	{"bmp\x30", "BM            \x30\x00\x00\x00", "image/bmp", all},
	{"bmp\x18", "BM            \x18\x00\x00\x00", "image/bmp", none},
	{"bmp\x10", "BM            \x10\x00\x00\x00", "image/bmp", none},
	{"bmp\x40", "BM            \x40\x00\x00\x00", "image/bmp", none},
	{"bmp\x34", "BM            \x34\x00\x00\x00", "image/bmp", none},
	{"bmp\x38", "BM            \x38\x00\x00\x00", "image/bmp", none},
	{"bmp\x28", "BM            \x28\x00\x00\x00", "image/bmp", none},
...
a,"b`,
	...
	{"dxf1", "  0\x0ASECTION\x0A", "image/vnd.dxf", one},
	{"dxf2", "  0\x0D\x0ASECTION\x0D\x0A", "image/vnd.dxf", none},
	{"dxf3", "0\x0ASECTION\x0A", "image/vnd.dxf", none},
	{"dxf4", "0\x0D\x0ASECTION\x0D\x0A", "image/vnd.dxf", none},
	...
	{"fm", "<MakerFile \x00", "application/vnd.framemaker", one},
	{"fmdict", "<MakerDictionary \x00", "application/vnd.framemaker", one},
	{"fmbook", "<BooKFilE \x00", "application/vnd.framemaker", one},
	...
	{"grib1", "GRIB\x00\x00\x00\x01", "application/grib", one},
	{"grib2", "GRIB\x00\x00\x00\x02", "application/grib", none},
	...
	{"ilbm", "FORM\x00\x00\x00\x00ILBM\x00", "image/x-ilbm", one},
	{"lbm", "FORM\x00\x00\x00\x00PBM \x00", "image/x-ilbm", one},
	{"inf os2", "HSP\x01\x9b\x00", "application/x-os2-inf", none},
	{"hlp os2", "HSP\x10\x9b\x00", "application/x-os2-hlp", none},
	{"it", "IMPM", "audio/x-it", one},
	...
	{"jar executable", "PK\x03\x04\x00\x00\x00\x00\x01" + offset(0x15, "\xFE\xCA"), "application/java-archive", none},
	...
	{"koala", "\x00\x60" + strings.Repeat("\x00", 10001), "image/x-koala-painter", one},
	{"lha", offset(2, "-lh5-") + "\x00", "application/x-lzh-compressed", one},
	...
	{"mod", offset(1080, "M.K."), "audio/x-mod", one},
	{"mov", "\x00\x00\x00\x14\x66\x74\x79\x70\x71\x74\x20\x20", "video/quicktime", one},
	{"maud", "FORM\x00\x00\x00\x00MAUD\x00", "audio/x-maud", one},
	...
	{"msa", "\x0E\x0F", "application/x-magic-shadow-archiver", one},
...
ENDHDR`,
	...
	{"s3m", offset(44, "SCRM") + "\x00", "audio/x-s3m", one},
	{"sh", "#!/bin/sh", "text/x-shellscript", one},
	{"sid", "PSID", "audio/prs.sid", one},
	...
	{"st", "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x02\x00\x01\x00\x01\x00", "application/x-atari-disk-image", one},
...
`,
	...
	{"xm", "Extended Module: ", "audio/x-xm", one},
...
func TestDetect(t *testing.T) {
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			if tc.name == "koala" {
				SetLimit(uint32(len(tc.data))) //nolint:gosec
